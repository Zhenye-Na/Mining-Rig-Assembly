...
public class RAM {
	...
	public static void main(String[] args) throws IOException {
		...
		Document d = Jsoup.parse(new File("C:\\Users\\noden\\Documents\\r52.html"), "UTF-8");
		Writer writer = new FileWriter("c:\\data\\RAM.txt", true);
		...
		for(Element row : rows) {
			...
			if(pricediv != null && pricediv.text().contains("$")){
				...
				for(Element div : divlist) {
					if(div.text().toLowerCase().contains("(") && div.text().toLowerCase().contains(")") && (div.text().toLowerCase().contains("gb") || div.text().toLowerCase().contains("mb")))	{
						System.out.println("Capacity: " + div.text());
						writer.write("Capacity: " + div.text() + "\n");
					}
				}
